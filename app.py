...
print("🤖 جاري تهيئة البوت الذكي بنظام المعالج المفتوح واللوجات الحية...", flush=True)
...
# --------------------------------------------------------
# 🎯 المُعالج المفتوح لجميع رسائل البوت (مع طباعة حية للوجات)
# --------------------------------------------------------
@bot.on_message()
async def main_bot_router(client: Client, message: Message):
    try:
        # 🔍 طباعة حية في لوحات ريلواي للتأكد من وصول الإشارة للبوت
        print(f"📥 [تحديث جديد] استقبلت رسالة من: {message.from_user.id if message.from_user else 'مجهول'} | النص: {message.text}", flush=True)
        ...
        if not state:
            # إذا أرسل نص عادي بدون أن يبدأ بـ /start وبدون حالة سابقة
